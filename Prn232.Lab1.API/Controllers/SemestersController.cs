...
public class SemestersController : ControllerBase
{
    ...
    {
        var result = await _service.GetSemestersAsync(search, sortBy, isDescending, page, pageSize);
        return Ok(ApiResult<Pagination<SemesterResponse>>.Success(result));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetSemester(int id, [FromQuery] string? expand = null)
    {
        var semester = await _service.GetSemesterByIdAsync(id, expand);
